...
| 🚚 **Переміщення** | Внутрішнє переміщення між підрозділами та МВО (Акт переміщення ОЗ) |
...
| 🚚 Акт переміщення ОЗ | Наказ №818 | PDF + Excel |
...
│   │   │   ├── TransfersPage.tsx       #    🚚 Переміщення ОЗ
...
Система має **19 моделей** у додатку `assets`:
...
| 🚚 `AssetTransfer` | Переміщення (акт) | document_number, document_date, from_location, to_location, from_person, to_person |
| 📋 `AssetTransferItem` | Рядок переміщення | transfer, asset, book_value, quantity |
...
| `CRUD` | `/assets/transfers/` | 🚚 Переміщення ОЗ |
...
| `GET` | `/documents/transfer/{id}/act/` | Наказ №818 | 🚚 Акт переміщення ОЗ |
...
### 🚚 Акт внутрішнього переміщення ОЗ (Наказ Мінфіну №818)
Оформлює внутрішнє переміщення ОЗ між підрозділами. Включає: таблицю переміщуваних об'єктів (назва, інв. номер, кількість, вартість), дані про напрямок переміщення, підписи (Здав / Прийняв / Гол. бухгалтер).
...
| Переоцінки / Поліпшення / Переміщення | ✅ | ✅ | ❌ |
