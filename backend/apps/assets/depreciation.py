...
def calc_straight_line(initial_cost, residual_value, useful_life_months):
    ...
    Амортизація = (Первісна вартість - Ліквідаційна вартість) / Строк (міс.)
    """
...
def calc_cumulative(initial_cost, residual_value, useful_life_months, months_used):
    ...
    Річна амортизація = (Первісна - Ліквідаційна) * Кумулятивний коефіцієнт
    Місячна = Річна / 12
    """
    if useful_life_months <= 0:
        return Decimal('0.00')
...
def calc_production(initial_cost, residual_value, total_capacity, monthly_volume):
    """
    5. Виробничий метод.
    Амортизація = (Первісна - Ліквідаційна) / Загальний обсяг * Обсяг за місяць
    """
