...
# ☁️ Google Drive Backup (опціонально)
...
Redis офіційно не підтримує Windows, але є три варіанти:
...
#### Варіант C: Redis для Windows (готова збірка) 📦

Завантаження скомпільованої версії Redis, яка працює нативно на Windows без WSL:

1. Перейдіть на GitHub: [https://github.com/tporadowski/redis/releases](https://github.com/tporadowski/redis/releases)
2. Завантажте останній реліз — файл `Redis-x64-*.msi` (інсталятор) або `Redis-x64-*.zip` (портативна версія)
3. **Варіант MSI (рекомендовано):**
   - Запустіть інсталятор `Redis-x64-*.msi`
   - Оберіть "Add the Redis installation folder to the PATH environment variable"
   - Оберіть "Configure Redis as a service" — Redis запускатиметься автоматично
   - Завершіть встановлення
4. **Варіант ZIP (портативна):**
   - Розпакуйте архів у зручне місце, наприклад `C:\Redis`
   - Додайте `C:\Redis` до змінної середовища PATH

**🔧 Запуск як служба Windows (для ZIP-варіанту):**

Якщо ви обрали ZIP і хочете, щоб Redis запускався автоматично з Windows — зареєструйте його як службу:

```cmd
# Відкрийте CMD від імені адміністратора!

# 1. Встановити Redis як службу Windows
...
# Видалити службу (якщо більше не потрібна)
redis-server --service-uninstall
```

Також можна керувати через стандартний менеджер служб Windows:
```cmd
# Через команду sc
sc start Redis
sc stop Redis
...
# Або відкрити графічний менеджер служб:
services.msc
# Знайдіть "Redis" у списку → ПКМ → Запустити / Зупинити
# Тип запуску: "Автоматично" — Redis стартуватиме разом з Windows
```
...
> 💡 **Примітка:** Ця збірка базується на Redis 5.0. Вона стабільна і повністю достатня для роботи Celery у цьому проекті. При встановленні через MSI служба створюється автоматично — додаткові кроки не потрібні.
...
### 2.12 🔴 Запуск Celery (опціонально)

Celery потрібен для фонових задач: автоматичні сповіщення, перевірка зносу ОЗ, резервне копіювання на Google Drive.

> ⚠️ **Вимоги:** Redis повинен бути запущений (див. розділ 2.4)

**Використання BAT-скрипта (рекомендовано):**
```cmd
:: Двічі клікніть на файл start-celery.bat
:: або з командного рядка:
start-celery.bat
```

**Або вручну** (два вікна командного рядка):

**🪟 Вікно 3 — Celery Worker:**
```cmd
cd Buh\backend
venv\Scripts\activate
...
**🪟 Вікно 4 — Celery Beat (планувальник):**
```cmd
cd Buh\backend
venv\Scripts\activate
...
> 💡 **Без Celery** система працює, але не виконуються:
> - Автоматичні сповіщення про необхідність нарахування амортизації
> - Перевірка ОЗ зі зносом >90%
> - Автоматичне резервне копіювання на Google Drive

### 2.13 ☁️ Налаштування Google Drive Backup (опціонально)

Для хмарного резервного копіювання бази даних на Google Drive:

1. Створіть проект у [Google Cloud Console](https://console.cloud.google.com/)
2. Увімкніть **Google Drive API**
3. Створіть **OAuth 2.0 Client ID** (тип: Desktop application)
4. Завантажте JSON з credentials
5. Додайте в `backend/.env`:
   ```env
   GDRIVE_CREDENTIALS_PATH=path/to/credentials.json
   GDRIVE_FOLDER_ID=your-google-drive-folder-id
   ```
6. Виконайте аутентифікацію:
   ```cmd
   cd Buh\backend
   venv\Scripts\activate
   python manage.py gdrive_auth
   ```
7. Дотримуйтесь інструкцій у терміналі (відкриється браузер для авторизації)

> 💡 `GDRIVE_FOLDER_ID` — це ID папки на Google Drive, куди зберігатимуться бекапи. Знайдіть його в URL папки: `https://drive.google.com/drive/folders/<FOLDER_ID>`
...
| Celery не запускається | Чи запущений Redis? `redis-cli ping` → PONG? |
| Сповіщення не надходять | Чи запущені Celery Worker і Beat? |
| Google Drive помилка | Чи є `GDRIVE_CREDENTIALS_PATH` в `.env`? Чи виконано `gdrive_auth`? |
